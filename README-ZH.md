...
## 简介
这是一个高级翻译插件，可用于自动翻译基于Unity的游戏，同时提供手动翻译所需的工具。

它会（显然地）连接到互联网以提供自动翻译，如果你对此感到不适，请不要使用。

如果你打算将此插件作为游戏翻译套件的一部分进行再分发，请阅读 [此章节](#regarding-redistribution)和[手动翻译](#manual-translations)章节以了解插件的运作方式。

## 插件框架
该模组可以无需任何外部依赖独立安装，也可作为以下插件管理器/模组加载器的插件使用：
...
### 独立安装 (ReiPatcher)
要求：无需其他工具，ReiPatcher已包含在本下载中。

*重要提示：使用此方法只需两次点击即可在大多数Unity游戏中使插件正常工作。如果使用了支持的插件管理器，应避免使用此安装方法，否则会导致问题。*

 0. 阅读上面的 `重要提示`。
 1. 从 [releases](../../releases) 下载 XUnity.AutoTranslator-ReiPatcher-{VERSION}.zip。
 2. 将其直接解压到游戏目录中，使 "SetupReiPatcherAndAutoTranslator.exe" 与其他exe文件放在一起。
 3. 执行 "SetupReiPatcherAndAutoTranslator.exe"，这将正确设置ReiPatcher。
 4. 执行在现有可执行文件旁创建的快捷方式 {GameExeName} (Patch and Run).lnk，这将修补并启动游戏。
 5. 从此以后，你可以直接通过 {GameExeName}.exe 启动游戏。
 6. 由于各种原因，并非所有文本钩子默认都启用。如果你发现游戏或部分内容没有被正确翻译，建议进入配置文件并启用某些被禁用的文本框架。配置文件在游戏首次启动时自动创建。
...
**注意：** ReiPatcher 目录中的 `Mono.Cecil.dll` 与 Managed 目录中的不是同一个文件。

### BepInEx 插件
要求：[BepInEx 插件管理器](https://github.com/BepInEx/BepInEx)（请先按它的安装说明进行安装！） 

 1. 从 [releases](../../releases) 下载 XUnity.AutoTranslator-BepInEx-{VERSION}.zip。
 2. 直接解压到游戏目录，使插件dll文件放置在BepInEx文件夹中。
 3. 启动游戏。
 4. 由于各种原因，并非所有文本钩子默认都启用。如果发现游戏未正确翻译，请进入配置文件启用被禁用的文本框架。配置文件在游戏首次启动时自动创建。
...
#### BepInEx IL2CPP 插件
IL2CPP 的安装说明与标准版本相同，只是你需要安装 BepInEx 6 for IL2CPP 版本（截至本文撰写时，仅可在[此处](https://builds.bepis.io/projects/bepinex_be)获取最新开发构建），并且需要使用本插件的 `BepInEx-IL2CPP` 包。

当前版本 (5.4.0) 是基于开发构建 704 编译的。

### MelonLoader 插件
要求：[Melon Loader](https://melonwiki.xyz)（请先按它的安装说明进行安装！）

 1. 从 [releases](../../releases) 下载 XUnity.AutoTranslator-MelonMod-{VERSION}.zip。
 2. 直接解压到游戏目录，使插件dll文件放置在Mods和UserLibs文件夹中。
 3. 启动游戏。
 4. 由于各种原因，并非所有文本钩子默认都启用。如果发现游戏未正确翻译，请进入配置文件启用被禁用的文本框架。配置文件在游戏首次启动时自动创建。
...
IL2CPP 的安装说明与标准版本相同，只是你需要使用 `MelonMod-IL2CPP`  包来代替。

### IPA 插件
REQUIRES: [IPA plugin manager](https://github.com/Eusth/IPA) （请先按它的安装说明进行安装！）

 1. 从 [releases](../../releases) 下载 XUnity.AutoTranslator-IPA-{VERSION}.zip。
 2. 直接解压到游戏目录，使插件dll文件放置在Plugins文件夹中。
 3. 启动游戏。
 4. 由于各种原因，并非所有文本钩子默认都启用。如果发现游戏未正确翻译，请进入配置文件启用被禁用的文本框架。配置文件在游戏首次启动时自动创建。
...
### UnityInjector 插件
要求：UnityInjector （请先按它的安装说明进行安装！）

 1. Download XUnity.AutoTranslator-UnityInjector-{VERSION}.zip from [releases](../../releases).
 2. 直接解压到游戏目录，使插件dll文件放置在UnityInjector文件夹中。 **This may not be game root directory!**
 3. 启动游戏。
 4. 由于各种原因，并非所有文本钩子默认都启用。如果发现游戏未正确翻译，请进入配置文件启用被禁用的文本框架。配置文件在游戏首次启动时自动创建。
...
### [Behaviour] 行为 Configuration Explanation

#### 空白字符处理
本节描述了执行翻译前后对空白字符处理有影响的配置参数。 **None of these settings have an impact on the 'untranslated texts' that are placed in the auto generated translations file.**

对于自动翻译来说，正确的空白字符处理至关重要。 控制空白字符处理的参数包括：
